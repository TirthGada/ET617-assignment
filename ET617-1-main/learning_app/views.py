...
from .models import Course, Content, Quiz, UserProgress, ClickstreamEvent, VideoAnalytics, TeacherProfile, LiveQuiz, QuizQuestion, QuizParticipant, QuizAnswer, StudentAnalysis, QuizAnalytics, Poll, PollOption, PollResponse, PollAnalytics
        ...
        if action == 'login':
            ...
            if not username:
                if email and '@' in email:
                    ...
                    username = f"teacher_{timezone.now().strftime('%Y%m%d%H%M%S')}"
...
def create_poll(request):
    ...
    if request.method == 'POST':
        title = request.POST.get('title')
        description = request.POST.get('description', '')
        poll_type = request.POST.get('poll_type', 'single_choice')
        is_anonymous = request.POST.get('is_anonymous') == 'on'
        allow_multiple = request.POST.get('allow_multiple_responses') == 'on'
...
def start_poll(request, poll_id):
    ...
    if poll.options.count() == 0 and poll.poll_type in ['single_choice', 'multiple_choice', 'yes_no']:
        messages.error(request, 'Cannot start poll without options!')
        return redirect('edit_poll', poll_id=poll.id)
...
def poll_monitor(request, poll_id):
    """Monitor active poll - see responses in real-time"""
    teacher_id = request.session.get('teacher_id')
    teacher_profile = get_object_or_404(TeacherProfile, id=teacher_id)
    poll = get_object_or_404(Poll, id=poll_id, teacher=teacher_profile)
    
    responses = poll.responses.all().order_by('-submitted_at')
    
    # Calculate analytics
    analytics = calculate_poll_analytics(poll)
    
    # Prepare chart data for template
    chart_data = []
    if poll.poll_type in ['single_choice', 'multiple_choice']:
        for option in poll.options.all():
            count = analytics['response_distribution'].get(option.option_text, 0)
    ...
    if request.GET.get('ajax') == '1':
        ...
        for response in responses[:10]:
            ...
            if poll.poll_type in ['single_choice', 'multiple_choice']:
                ...
                response_text = response.text_response[:50] + "..." if len(response.text_response) > 50 else response.text_response
            elif poll.poll_type == 'rating_scale':
                response_text = f"{response.rating_value}/5 stars"
            
            recent_responses_html += f"""
            <div class="d-flex justify-content-between align-items-center border-bottom py-2">
                <div>
                    <strong>{response.student_name or 'Anonymous'}</strong>
                    <small class="text-muted d-block">{response_text}</small>
                </div>
        ...
        return JsonResponse({
            ...
            'recent_responses': recent_responses_html
        })
...
def poll_results(request, poll_id):
    """View final poll results and analytics"""
    teacher_id = request.session.get('teacher_id')
    teacher_profile = get_object_or_404(TeacherProfile, id=teacher_id)
    poll = get_object_or_404(Poll, id=poll_id, teacher=teacher_profile)
...
def take_poll(request, poll_id):
    ...
    if request.method == 'POST':
        ...
        # Create response based on poll type
        response = PollResponse.objects.create(
            poll=poll,
            student_name=student_name if not poll.is_anonymous else '',
            student_email=student_email if not poll.is_anonymous else '',
            ip_address=get_client_ip(request)
        )
        ...
        if poll.poll_type in ['single_choice', 'multiple_choice']:
            ...
            response.rating_value = int(request.POST.get('rating_value', 0))
...
def calculate_poll_analytics(poll):
    """Calculate real-time poll analytics"""
    ...
    if poll.poll_type in ['single_choice', 'multiple_choice']:
        ...
        for option in poll.options.all():
            ...
            for response in responses:
                if str(option.id) in response.selected_options:
                    count += 1
            analytics['response_distribution'][option.option_text] = count
...
def generate_poll_analytics(poll):
    """Generate final analytics for ended poll"""
    analytics_data = calculate_poll_analytics(poll)
